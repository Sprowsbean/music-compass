...
export function usePipeline() {
  ...
  const addUser = useCallback(async (files: File[], name: string, dateOfBirth: string) => {
    ...
    setAstrology(null);
    setRegression(null);
    setProgress({ phase: 'Parsing Spotify history…', current: 0, total: 0, database: 0, gemini: 0 });
    ...
    try {
      ...
      setProgress({
        ...
        total:    parsed.songs.length,
        database: scored.filter(s => s.source?.includes('kaggle') || s.source === 'supabase_cache').length,
